...
(*****************************************************************************)
type instrn = { proc: int;      pc  : int;     op  : ikind; 
                var : string;   data: string;  wrID: int;  
                wrType: wtype;  wrProc: int;   reg : string; 
                useReg: bool;   id  : int } 


(************* the external file for generating tuples  ******************)
...
(*************************************************************)
(*  parse the command                                        *)
(*  Currently, mpec can just accept  name of the input file  *)
(*************************************************************) 
let get_input_file_name ()  =  
  let input_file_name = try  Sys.argv.(1)  with 
...
(**********************************************************************)
(* parse the input file, and extract information into two             *)
(* arrays. One is for tuples, the other is for initialization values. *)
(**********************************************************************)

let (array_tTups, start_values_list) =   
  let input_file = get_input_file_name () in
  let retval = gentuple_parse_input_file (input_file) in
    print_string "parse retval = "; print_int retval; 
    print_string "\n"; flush stdout;
    if (retval = 0) then
      ( (gentuple_get_tuple_array ()), (Array.to_list (gentuple_get_init_vals ())) )
      else 
	begin
	  print_string "Parse error ... exit \n";
	  flush stdout;
	  exit(0); ( [||], [] )
	end
...
(*********************   Debugging code  *******************)
let _ = match debug_tag with
    true -> 
      begin
	let len = Array.length array_tTups in
	let print_instr itp = 
	  begin
	    Printf.printf "{id=%d; proc=%d; pc=%d; " (itp.id) (itp.proc) (itp.pc) ; flush stdout;
	    ( match itp.op with
		  LdAcq -> Printf.printf "op=LdAcq"; flush stdout	
		| StRel -> Printf.printf "op=StRel"; flush stdout
		| Ld    -> Printf.printf "op=Ld";    flush stdout
		| St    -> Printf.printf "op=St";    flush stdout
		| Mf    -> Printf.printf "op=Mf";    flush stdout
	    );
	    Printf.printf " var=%s; data=%s; wrID=%d" (itp.var) (itp.data) (itp.wrID);  flush stdout;
	    
	    (match itp.wrType with
		 Local -> Printf.printf "wrType=Local; ";       flush stdout
               | Remote-> Printf.printf "wrType=Remote; ";      flush stdout
               | DontCare-> Printf.printf "wrType=DontCare; ";  flush stdout
	    );
	    
	    Printf.printf "wrProc=%d; reg=%s; };\n" (itp.wrProc) (itp.reg); flush stdout;
	    if (itp.useReg) then
	      ( Printf.printf "useReg=true;}\n"; flush stdout )
	    else
	      ( Printf.printf "useReg=false;}\n"; flush stdout );
	    ()
	  end
	in
	  for i = 1 to len do 
	    print_instr array_tTups.(i-1);
	    flush stdout
	  done
      end
  | _ -> ()
...
(*********************************************************************)
(*  Check whether there are x and -x in the same clause. If there    *)
(*  if such a pair of numbers, return true, otherwise return false   *)
(*********************************************************************)
let rec check_negate ilist = 
  match ilist with
      [] -> false
    | a :: ilist1 -> 
	(let cmp x = (x = (-a)) in
	 if   (List.exists cmp ilist1) then true
	 else check_negate ilist1
        )
...
and gate_clauses2str_and_out(gate_clauses)(cchan) =
  match gate_clauses with
    ...
    | (clause,{op1=op1;op2=op2;op3=op3;op4=op4;rule=rule})::gate_clauses1 ->
         if (check_negate clause) 
	 then ( gate_clauses2str_and_out (gate_clauses1)(cchan) )
	 else  
...
let final_cnf_file = "/home/ganesh/tmp/cnf_temp"

let cnf_generator() =
  let clause_file = "/home/ganesh/tmp/bCNF_temp" in
  let summary_file = "/home/ganesh/tmp/pCNF_temp"  in
  
  let _ = init_Trans() in  
  let start_time = 
          Printf.printf  "Generating CNF file ... \n "; 
          flush stdout; 
          Unix.gettimeofday ()          in 
  let cchan  = open_out   ( clause_file ) in
  let vnext  = 1 + var_at ( tLen ) (tLen) in

  let reflex_start  = print_string "... reflextive rules ... \n "; flush stdout ; Unix.gettimeofday() in   
  ...
  let _ =  Unix.system ("cat " ^ summary_file ^ " " ^ clause_file ^ " > " ^ final_cnf_file) in
...
let get_sat_model cnf_file_name = 
  ...
  let sat_result =    (* 1 -- unsatistiable, 2 -- satisfiable *) 
       Zchaff.zchaff_ReadCnf (mpec_solver) (cnf_file_name);
...
(*
let extract_one_core  file_name = 
  let _ = Unix.system("./zverify_df " ^ file_name ^ " resolve_trace -core"); Unix.wait() in
    try  
      let _ = Unix.stat "unsat_core.cnf" in  true
    with
	Unix.Unix_error (Unix.ENOENT, "stat", "unsat_core.cnf") ->    
	  Printf.fprintf stderr "unsat core file missing!!\n" ;
	  false
*)


let solve_cnf_file file_name = 
  let sat_solver = Zchaff.zchaff_InitManager () in
  let sat_result =     (* 1 -- unsatistiable, 2 -- satisfiable *) 
       Zchaff.zchaff_ReadCnf (sat_solver) (file_name);
       Printf.printf "\n**** zchaff egine ... read_cnf .... done \n\n"; flush stdout;
       Zchaff.zchaff_Solve (sat_solver)     in
  let _ =  Zchaff.zchaff_ReleaseManager (sat_solver) in
(*  let _ = Gc.full_major () in *)
  let _ =  Printf.printf "**********  zchaff solver is released ... *********** \n"; flush stdout in
(*  let _ =  Unix.stat "resolve_trace" in *)
    sat_result


let extract_one_core  file_name = 
  let _ = Unix.system ("rm resolve_trace") in
  let _ = Printf.printf "******* using zchaff to solve .... %s \n\n" file_name ; flush stdout in
  let _ = solve_cnf_file file_name in 
  let _ = flush stdout in
  let _ =  Unix.stat "resolve_trace" in 
  let _ = Unix.system ("./bin/zverify_df " ^ file_name ^ " resolve_trace -core > temp ")in
    true

(*
  let pid  =  Unix.fork() in
    if (pid = 0) then
      let _ = Unix.execvp "./zverify_df"  [|"./zverify_df"; file_name;  "resolve_trace"; "-core"|] in
      true
    else
      let _ = Unix.wait() in true
 *)

       
(* Use zcore or zverify_db to extract the core  *)

let iterate_num = ref 0
let file_list = [| |] 

let rec extract_unsat_core file_name = 
  let _ = Printf.printf ("\n\n************ iteration %d *******************\n\n")  (!iterate_num);
          flush stdout  in
  let new_file_name = "unsat_core" ^ string_of_int(!iterate_num) ^".cnf" in
  let reach_fixpoint = 
    let sat_tag = extract_one_core (file_name) in
    if ( sat_tag  ) then
      begin
	try
	  let _ = print_string  ("mv unsat_core.cnf " ^ new_file_name ^ "\n"); flush stdout in
	  let _ = Unix.system ("mv unsat_core.cnf " ^ new_file_name ) in
          let  _ = Array.append file_list [| new_file_name |] in
	  let f1 = Unix.stat (file_name) in
	  let _ = print_string ( "****** unsat_core" ^ string_of_int(!iterate_num) ^".cnf ************\n"  ); 
	          flush stdout in
	  let f2 = Unix.stat (new_file_name) in
  	  iterate_num := !iterate_num + 1;
	  Printf.printf "f1.size = %d   f2.size = %d \n"  f1.Unix.st_size  f2.Unix.st_size ; flush stdout;
	  if (f1.Unix.st_size = f2.Unix.st_size) 
	  then true
	  else false
	with
	    Unix.Unix_error (Unix.EOVERFLOW, "stat", file_name) -> 
	      iterate_num := !iterate_num + 1; false
      end
    else
      true
  in
    if (not reach_fixpoint)  then
      let _ = print_string ("*** " ^ new_file_name ^"  \n") in 
      extract_unsat_core ( new_file_name )
    else
      file_list
...
let main = 
  let _ = cnf_generator () in

  let _ = Printf.printf "final_cnf_file = %s\n" final_cnf_file; flush stdout in 
  let result = solve_cnf_file final_cnf_file in
  let _ =  Unix.stat "resolve_trace" in 

  let _ = Printf.printf "result = %d \n" result; flush stdout in


let file_list : string array = 
    if (result = 1) then  (extract_unsat_core (final_cnf_file); [| |] )
    else  [| |]               in 
    Printf.printf "iteratation = %d \n"  ( Array.length (file_list) )
